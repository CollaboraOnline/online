...
var CanvasSectionObject = /** @class */ (function () {
    function CanvasSectionObject(options) {
        this.context = null;
        this.myTopLeft = null;
        this.documentTopLeft = null; // Document top left will be updated by container.
...
var CanvasSectionContainer = /** @class */ (function () {
    function CanvasSectionContainer(canvasDOMElement) {
        /*
        ...
        */
        ...
        this.sections = new Array(0);
        this.documentTopLeft = [0, 0];
        this.positionOnMouseDown = null;
        this.positionOnMouseUp = null;
        this.positionOnClick = null;
        this.positionOnDoubleClick = null;
        this.mousePosition = null;
        this.dragDistance = null;
        this.draggingSomething = false; // This will be managed by container, used by sections.
        this.sectionOnMouseDown = null; // (Will contain section name) When dragging, user can leave section borders, dragging will continue. Target section will be informed.
        this.sectionUnderMouse = null; // For mouse enter & leave events.
        this.draggingTolerance = 5; // This is for only desktop, mobile browsers seem to distinguish dragging and clicking nicely.
        this.multiTouch = false;
        this.touchCenter = null;
        this.potentialLongPress = false;
        this.clearColor = 'white';
        this.touchEventInProgress = false; // This prevents multiple calling of mouse down and up events.
    ...
    CanvasSectionContainer.prototype.setClearColor = function (color) {
        this.clearColor = color;
    };
    CanvasSectionContainer.prototype.getClearColor = function () {
        return this.clearColor;
    };
    CanvasSectionContainer.prototype.clearMousePositions = function () {
    ...
    CanvasSectionContainer.prototype.convertPositionToSectionLocale = function (section, point) {
        return [point[0] - section.myTopLeft[0], point[1] - section.myTopLeft[1]];
    };
    CanvasSectionContainer.prototype.convertPositionToCanvasLocale = function (e) {
    ...
    CanvasSectionContainer.prototype.convertPointToCanvasLocale = function (point) {
        var rect = this.canvas.getBoundingClientRect();
        var x, y;
        x = point[0] - rect.left;
        y = point[1] - rect.top;
    ...
    CanvasSectionContainer.prototype.getSectionWithName = function (name) {
        if (name) {
    ...
    CanvasSectionContainer.prototype.getDocumentTopLeft = function () {
        return [this.documentTopLeft[0], this.documentTopLeft[1]];
    };
    CanvasSectionContainer.prototype.setDocumentTopLeft = function (point) {
    ...
    CanvasSectionContainer.prototype.requestReDraw = function () {
        this.drawSections();
    };
    CanvasSectionContainer.prototype.onClick = function (e) {
        if (!this.draggingSomething) { // Prevent click event after dragging.
            ...
            if (s1 && s2 && s1 == s2) { // Allow click event if only mouse was above same section while clicking.
                var section = this.findSectionContainingPoint(this.positionOnClick);
    ...
    CanvasSectionContainer.prototype.onDoubleClick = function (e) {
        this.positionOnDoubleClick = this.convertPositionToCanvasLocale(e);
        var section = this.findSectionContainingPoint(this.positionOnDoubleClick);
        if (section) {
    ...
    CanvasSectionContainer.prototype.onMouseMove = function (e) {
        if (!this.potentialLongPress) {
                ...
                if (section) {
                    if (section.name !== this.sectionUnderMouse) {
                        if (this.sectionUnderMouse !== null) {
                            var previousSection = this.getSectionWithName(this.sectionUnderMouse);
                            if (previousSection)
            ...
            this.mousePosition = this.convertPositionToCanvasLocale(e);
            var section = this.findSectionContainingPoint(this.mousePosition);
            if (section) {
    ...
    CanvasSectionContainer.prototype.onMouseDown = function (e) {
        if (!this.touchEventInProgress) {
            this.clearMousePositions();
            this.positionOnMouseDown = this.convertPositionToCanvasLocale(e);
            var section = this.findSectionContainingPoint(this.positionOnMouseDown);
            if (section) {
    ...
    CanvasSectionContainer.prototype.onMouseUp = function (e) {
        if (!this.touchEventInProgress) {
            this.positionOnMouseUp = this.convertPositionToCanvasLocale(e);
            if (!this.draggingSomething) {
                var section = this.findSectionContainingPoint(this.positionOnMouseUp);
                if (section) {
                    section.onMouseUp(this.convertPositionToSectionLocale(section, this.positionOnMouseUp), e);
                }
            }
            else {
                var section = this.getSectionWithName(this.sectionOnMouseDown);
                if (section) {
                    section.onMouseUp(this.convertPositionToSectionLocale(section, this.positionOnMouseUp), e);
                }
            }
    ...
    CanvasSectionContainer.prototype.onMouseWheel = function (e) {
        var point = this.convertPositionToCanvasLocale(e);
        var delta = e.deltaY;
        var section = this.findSectionContainingPoint(point);
        if (section)
            section.onMouseWheel(this.convertPositionToSectionLocale(section, point), delta, e);
    };
    CanvasSectionContainer.prototype.onMouseLeave = function (e) {
        if (this.sectionUnderMouse !== null) {
    ...
    CanvasSectionContainer.prototype.onTouchStart = function (e) {
    ...
    CanvasSectionContainer.prototype.onTouchMove = function (e) {
        this.potentialLongPress = false;
        if (!this.multiTouch) {
            this.mousePosition = this.convertPositionToCanvasLocale(e);
            this.draggingSomething = true;
    ...
    CanvasSectionContainer.prototype.onTouchEnd = function (e) {
        this.potentialLongPress = false;
        if (!this.multiTouch) {
            this.positionOnMouseUp = this.convertPositionToCanvasLocale(e);
            if (!this.draggingSomething) {
    ...
    CanvasSectionContainer.prototype.onTouchCancel = function (e) {
        this.clearMousePositions();
        this.potentialLongPress = false;
    };
    CanvasSectionContainer.prototype.onResize = function (newWidth, newHeight) {
        this.dpiScale = window.devicePixelRatio;
        ...
        var cssWidth = newWidth / this.dpiScale; // NB. beware
    ...
    CanvasSectionContainer.prototype.findSectionContainingPoint = function (point) {
        for (var i = this.sections.length - 1; i > -1; i--) { // Search from top to bottom. Top section will be sent as target section.
            if (this.sections[i].interactable && this.doesSectionIncludePoint(this.sections[i], point))
                return this.sections[i];
        }
        return null;
    };
    CanvasSectionContainer.prototype.doesSectionIncludePoint = function (section, point) {
    ...
    CanvasSectionContainer.prototype.doSectionsIntersectOnYAxis = function (section1, section2) {
    ...
    CanvasSectionContainer.prototype.doSectionsIntersectOnXAxis = function (section1, section2) {
    ...
    CanvasSectionContainer.prototype.hitLeft = function (section) {
        var maxX = -1;
        for (var i = 0; i < this.sections.length; i++) {
    ...
    CanvasSectionContainer.prototype.hitRight = function (section) {
        var minX = Infinity;
        for (var i = 0; i < this.sections.length; i++) {
            if (this.sections[i].zIndex === section.zIndex && this.sections[i].name !== section.name) {
    ...
    CanvasSectionContainer.prototype.hitTop = function (section) {
        var maxY = -1;
        for (var i = 0; i < this.sections.length; i++) {
            if (this.sections[i].zIndex === section.zIndex && this.sections[i].name !== section.name) {
    ...
    CanvasSectionContainer.prototype.hitBottom = function (section) {
        var minY = Infinity;
        for (var i = 0; i < this.sections.length; i++) {
            if (this.sections[i].zIndex === section.zIndex && this.sections[i].name !== section.name) {
    ...
    CanvasSectionContainer.prototype.reNewAllSections = function (redraw) {
    ...
    CanvasSectionContainer.prototype.locateSections = function () {
        for (var i = 0; i < this.sections.length; i++) {
    ...
    CanvasSectionContainer.prototype.orderSections = function () {
    ...
    CanvasSectionContainer.prototype.applyDrawingOrders = function () {
    ...
    CanvasSectionContainer.prototype.drawSectionBorders = function () {
    ...
    CanvasSectionContainer.prototype.drawSections = function () {
    ...
    CanvasSectionContainer.prototype.doesSectionExist = function (name) {
    ...
    CanvasSectionContainer.prototype.newSectionChecks = function (options) {
    ...
    CanvasSectionContainer.prototype.addSection = function (options, parentSectionName) {
        if (parentSectionName === void 0) { parentSectionName = null; }
        if (this.newSectionChecks(options)) {
            ...
            var newSection = new CanvasSectionObject(options);
    ...
    CanvasSectionContainer.prototype.removeSection = function (name) {
        var found = false;
        for (var i = 0; i < this.sections.length; i++) {
            if (this.sections[i].name === name) {
