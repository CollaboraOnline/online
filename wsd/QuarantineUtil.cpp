...
void Quarantine::makeQuarantineSpace(std::size_t headroomBytes)
{
    ...
    while (!entries.empty())
    {
        ...
        LOG_DBG("Removing quarantined file [" << COOLWSD::anonymizeUrl(entry.filename()) << "] for [" << Anonymizer::anonymize(entry.docKeyNoLog())
...
bool Quarantine::quarantineFile(const std::string& docPath)
{
    ...
    {
        LOG_WRN("Failed to quarantine [" << COOLWSD::anonymizeUrl(docPath) << "] for docKey [" << Anonymizer::anonymize(_docKeyNoLog)
                                         << "]: " << exc.what());
    }
...
Quarantine::Entry::Entry(const std::string& root, const std::string& filename)
{
    ...
    LOG_TRC("Legacy quarantine file for [" << Anonymizer::anonymize(_docKeyNoLog) << "], name: [" << COOLWSD::anonymizeUrl(_filename) << "], size: "
                         ...
                         const std::string& filename)
{
    ...
    std::vector<StringToken> tokens;
    StringVector::tokenize(filename.c_str(), filename.size(), Delimiter, tokens);
    LOG_TRC("Quarantine file for [" << Anonymizer::anonymize(_docKeyNoLog) << "], name: [" << COOLWSD::anonymizeUrl(filename)
                                    << "]: " << tokens.size());
    if (tokens.size() >= 3)
    {
        _secondsSinceEpoch =
    ...
    LOG_TRC("Quarantine file for [" << Anonymizer::anonymize(_docKeyNoLog) << "], name: [" << COOLWSD::anonymizeUrl(_filename) << "], size: " << _size
                         ...
                         uint64_t secondsSinceEpoch, const std::string& filename, uint64_t size)
{
    ...
    LOG_TRC("New quarantine file for [" << Anonymizer::anonymize(_docKeyNoLog) << "], name: [" << COOLWSD::anonymizeUrl(_filename) << "], size: "
                                        << _size << ", created: " << _secondsSinceEpoch);
}
