                                                ...
                                                const Poco::JSON::Object::Ptr& remoteJson)
{
    ...
    if (!remoteJson->isObject(indrectionEndpointKey))
    {
    ...
    if (JsonUtil::findJSONValue(geolocationSetupObj, "enable", geolocationEnabled))
    {
        ...
        newAppConfig.insert(
            std::make_pair(geolocationKey + ".enable", boolStr));
    }
    ...
    Poco::JSON::Array::Ptr allowedOrigins = geolocationSetupObj->getArray("allowed_websocket_origins");
