...
size_t COOLWSD::addNewChild(std::shared_ptr<ChildProcess> child)
{
...
void COOLWSD::removeUnusedChild(const std::shared_ptr<ChildProcess>& child)
{
    std::string configId;
    std::unique_lock<std::mutex> lock(NewChildrenMutex);
...
// NOTĂ: socket este transmis prin valoare (shared_ptr), FĂRĂ const&
void COOLWSD::handleForkitConnection(std::shared_ptr<StreamSocket> socket,
