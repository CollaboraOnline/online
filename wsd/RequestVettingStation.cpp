                                          ...
                                          bool isReadOnly, int redirectLimit)
{
         ...
         redirectLimit](const std::shared_ptr<http::Session>& session)
    {
        ...
        createDocBroker(docKey, url, uriPublic, isReadOnly, wopiInfo);
    };
                                            ...
                                            Poco::JSON::Object::Ptr wopiInfo)
{
    ...
    docBroker->setupTransfer(
        ...
        [docBroker, clientSession, uriPublic, wopiInfo,
         this](const std::shared_ptr<Socket>& moveSocket)
        {
            try
            {
                ...
                if (wopiInfo)
                {
                    ...
                    // if (_wopiInfo->getSupportsLocks())
                    //     _lockCtx.initSupportsLocks();
                }

                // Add and load the session.
                // Will download synchronously.
