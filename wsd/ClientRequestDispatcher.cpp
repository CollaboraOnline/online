    ...
    const std::shared_ptr<StreamSocket>& socket)
{
                          ...
                          (const std::shared_ptr<http::Session>& probeSession)
    {
        ...
        LOG_DBG_S("Wopi Access Check: got response state: " << responseState << " "
        ...
        if (sslResult != X509_V_OK)
        {
            if (sslResult == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT) {
                ...
                status = CheckStatus::CertificateValidation;
                LOG_WRN_S("Unexpected failed Result ssl in a connection success: " << probeSession->getSslVerifyMessage());
            }
        }
#endif
