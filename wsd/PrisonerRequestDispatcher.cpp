...
extern std::mutex NewChildrenMutex;
extern std::vector<std::shared_ptr<ChildProcess>> NewChildren;
extern std::map<std::string, std::shared_ptr<ForKitProcess>> SubForKitProcs;
extern std::map<std::string, std::chrono::steady_clock::time_point> LastForkRequestTimes;
extern std::shared_ptr<class PrisonPoll> PrisonerPoll;

extern size_t addNewChild(std::shared_ptr<ChildProcess> child);
#if !MOBILEAPP
extern void rebalanceChildren(const std::string& configId, int64_t balance);
#endif
...
void PrisonerRequestDispatcher::handleIncomingMessage(SocketDisposition &disposition) override
    ...
    class PrisonerSocketFactory final : public SocketFactory
        ...
        std::shared_ptr<Socket> PrisonerSocketFactory::create(const int fd, Socket::Type type)
        {
            // No local delay.
