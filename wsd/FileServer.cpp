...
bool FileServerRequestHandler::isAdminLoggedIn(const HTTPRequest& request, http::Response& response)
{
                           ...
                           const std::shared_ptr<StreamSocket>& socket)
    {
        ...
        if (request.getMethod() == "GET" && !path.toString().ends_with(suffix))
        {
            ...
            Poco::JSON::Object::Ptr userSettingsInfo = new Poco::JSON::Object();
            std::string uri = COOLWSD::getServerURL() + "/wopi/settings/sharedconfig.json";
            userSettingsInfo->set("SharedSettings", Util::trim(uri));
            uri = COOLWSD::getServerURL() + "/wopi/settings/userconfig.json";
            userSettingsInfo->set("UserSettings", Util::trim(uri));
            // authentication token to get them ??
            fileInfo->set("SettingsJSON", userSettingsInfo);
