...
bool ClientSession::handleKitToClientMessage(const std::shared_ptr<Message>& payload)
{
    ...
    {
        std::optional<bool> result = handleDocNotPassProtected(payload, docBroker, saveAsSocket);
        if (result)
        {
            return *result;
        }
...
std::optional<bool>
ClientSession::handleDocNotPassProtected(const std::shared_ptr<Message>& payload,
                                         const std::shared_ptr<DocumentBroker>& docBroker,
