            ...
            if (valid)
            {
                try
                {
                    ...
                    if (statusCode == http::StatusCode::OK)
                    {
                        ...
                        LOG_DBG("Got " << body.size() << " bytes for "
                                       << remoteServerURI.toString());
...
public:
                                 ...
                                 Poco::JSON::Object::Ptr remoteJson)
    {
        try
        {
            ...
            for (i = 0; i < lockedHostPatterns->size(); i++)
            {
                ...
                newAppConfig.insert(
                    std::make_pair(path + "[@read_only]", booleanToString(readOnly)));
                                  ...
                                  const Poco::JSON::Object::Ptr& remoteJson)
    {
        ...
        {
            LOG_INF(
                "Overriding indirection_endpoint.url failed because the indirection_endpoint.url "
                "entry does not "
                "exist");
        }
        catch (const std::exception& exc)
...
private:
    bool downloadPlain(const std::string& uri)
    {
        ...
        std::shared_ptr<http::Session> httpSession(
            StorageConnectionManager::getHttpSession(fontUri));
