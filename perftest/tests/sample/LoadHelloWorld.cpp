...
class LoadHelloWorld : public CyclePerfTest
{
public:
    LoadHelloWorld(const std::string &server) :
        CyclePerfTest("LoadHelloWorld", server)
    {
    }
...
//sendMessage("uno .uno:SidebarHide");
//sendMessage("commandvalues command=.uno:ViewAnnotations");
        //sleep(100);
        //sleep(100);
        waitForIdle();
...
{
    return std::make_shared<LoadHelloWorld>(server);
}
