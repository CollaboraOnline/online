...
export class CommentSection extends CanvasSectionObject {
	...
	constructor () {
		...
		this.sectionProperties.commentsAreListed = (app.map._docLayer._docType === 'text' || app.map._docLayer._docType === 'presentation' || app.map._docLayer._docType === 'drawing') && !window.mode.isSmallScreenDevice();
	...
	private checkCollapseState(): void {
		if (!window.mode.isSmallScreenDevice() && app.map._docLayer._docType !== 'spreadsheet') {
	...
	private checkSize (): void {
		...
		if (app.map._docLayer._docType === 'spreadsheet' || window.mode.isSmallScreenDevice() || this.sectionProperties.commentList.length === 0)
		{
	...
	public shouldCollapse (): boolean {
		if (app.map._docLayer._docType === 'text')
			return false;
		if (!this.containerObject.getDocumentAnchorSection() || app.map._docLayer._docType === 'spreadsheet' || window.mode.isSmallScreenDevice())
			return false;
			...
			build: function ($trigger: any) {
				...
				return {
					...
					items: {
						...
						showBigger: docLayer._docType !== 'text' || window.mode.isSmallScreenDevice() ? undefined : {
							name: isShownBig ? _('Show on the side') : _('Open in full view'),
							callback: function (key: any, options: any) {
								this.toggleShowBigger.call(this, options.$trigger[0].annotation);
							}.bind(this)
						},
						showInNavigator: (docLayer._docType !== 'text' && docLayer._docType !== 'spreadsheet') || window.mode.isSmallScreenDevice() ? undefined : {
