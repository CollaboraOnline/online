...
export class ScrollSection extends CanvasSectionObject {
	...
	*/
	...
	public onMouseWheel (point: Array<number>, delta: Array<number>, e: WheelEvent): void {
		...
		if (shouldAnimate) {
			if (Math.max(Math.abs(hscroll), Math.abs(vscroll)) % 1 !== 0) {
				// In Firefox, touchpad events have a fractional component.
				shouldAnimate = false;
			}
			else if (Math.abs((e as any).wheelDelta) % 120 !== 0 &&
					 (e as any).wheelDeltaX == -e.deltaX &&
					 (e as any).wheelDeltaY == -e.deltaY) {
				// In Chrome, wheelDeltaX and wheelDeltaY are always equal to negative deltaX and
				// deltaY and in the case of wheel events, are always multiples of 120. Unfortunately,
				// it is possible, though unlikely, for this to hit a false-positive.
				// Note that WheelEvent.wheelDelta* are deprecated.
				shouldAnimate = false;
			}
		}
