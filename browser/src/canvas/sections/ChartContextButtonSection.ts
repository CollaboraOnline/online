...
class ChartContextButtonSection extends HTMLObjectSection {
	...
	constructor(buttonType: number) {
		super(
			...
			new cool.SimplePoint(
				GraphicSelection.rectangle.x2 + 5 * 16,
				GraphicSelection.rectangle.y1 + buttonType * (32 + 5) * 16,
			),
	...
	updatePosition(): void {
		...
		if (this.lastZoom != origZoom) {
			...
			var zoomValue = app.map.zoomToFactor(origZoom);
			var x = Math.round(
				(GraphicSelection.rectangle.x2 + (5 * 16) / zoomValue) *
					app.twipsToPixels,
			);
			var y = Math.round(
				(GraphicSelection.rectangle.y1 +
					(this.sectionProperties.buttonType * (32 + 5) * 16) / zoomValue) *
					app.twipsToPixels,
			);
