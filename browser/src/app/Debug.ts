...
class DebugManager {
	...
	private _addDebugTools(): void {
		...
		this._addDebugTool({
			...
			onRemove: function () {
				...
				const keys = Object.keys(self._overlayData);
				for (const i of keys) {
	...
	public setOverlayMessage(id: string, message: string): void {
		if (this.overlayOn) {
			if (!this._overlayData[id]) {
				const topLeftNames = ['tileData', 'eventDelayTime'];
				const position = topLeftNames.includes(id) ? 'topleft' : 'bottomleft';
	...
	private _tileDataShowOverlay(): void {
	...
	public reportEventDelay(delayMs: number): void {
		...
		if (
			...
			this.setOverlayMessage(
				'eventDelayTime',
				'Event handling delay: ' + delayMs + 'ms',
			);
