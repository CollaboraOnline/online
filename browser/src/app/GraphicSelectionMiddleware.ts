...
class GraphicSelection {
	...
	public static updateGraphicSelection() {
		if (this.hasActiveSelection()) {
			...
			else if (extraInfo.id !== this.handlesSection.sectionProperties.info.id) { // Another shape is selected.
				this.handlesSection.removeSubSections();
				app.sectionContainer.removeSection(this.handlesSection.name);
				this.handlesSection = null;
				addHandlesSection = true;
		...
		else if (this.handlesSection && app.sectionContainer.doesSectionExist(this.handlesSection.name)){
			this.handlesSection.removeSubSections();
			app.sectionContainer.removeSection(this.handlesSection.name);
			this.handlesSection = null;
		}
    ...
    public static onMessage (textMsg: string) {
		...
		if (textMsg.match('EMPTY')) {
			...
			if (!window.ThisIsTheiOSApp && this.extraInfo.isDraggable && !this.extraInfo.svg && !isFrame)
			{
				app.socket.sendMessage('rendershapeselection mimetype=image/svg+xml');
			}

			// scroll to selected graphics, if it has no cursor
			if (!app.map._docLayer.isWriter() && this.rectangle
				&& app.map._docLayer._allowViewJump()) {

				if (
					(
						!app.isPointVisibleInTheDisplayedArea([this.rectangle.x1, this.rectangle.y1]) ||
						!app.isPointVisibleInTheDisplayedArea([this.rectangle.x2, this.rectangle.y2])
					)
					&&
