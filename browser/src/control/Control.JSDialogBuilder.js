...
window.L.Control.JSDialogBuilder = window.L.Control.extend({
	...
	_parseSpinFieldValue: function(displayValue) {
		...
		return displayValue.replace(/[^\d.\-]/g, '').trim();
	},
	...
	baseSpinField: function(parentContainer, data, builder, customCallback) {
		...
		up.type = 'button';
		up.tabIndex = -1;
		up.setAttribute('aria-label', 'Increment value');
		var down = window.L.DomUtil.create('button', cssClass + ' spinfieldbutton-down', buttons);
		down.type = 'button';
		down.tabIndex = -1;
		down.setAttribute('aria-label', 'Decrement value');
	...
	executeActionImpl: function(container, data) {
		...
		case 'setText':
			...
			var spinContainer = innerInput ? control : null;
			if (innerInput)
				control = innerInput;

			var currentText = this._cleanText(data.text);
			if (spinContainer && spinContainer._unit)
				currentText = this._formatSpinFieldValue(currentText, spinContainer._unit);
			if (spinContainer)
