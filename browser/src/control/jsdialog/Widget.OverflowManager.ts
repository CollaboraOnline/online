...
class OverflowManager {
	...
	hasOverflow(maxWidth: number): boolean {
		const requiredWidth = this.parentContainer.clientWidth;
		console.debug('overflow max: ' + maxWidth + ' req: ' + requiredWidth);
	...
	onResize() {
		...
		this.overflowMenuDebounced = setTimeout(() => {
			app.layoutingService.appendLayoutingTask(() => {
				...
				// then hide required TODO: reversed order
				groups.forEach((element: OverflowGroupContainer) => {
...
JSDialog.OverflowManager = function (
	...
	const manager = new OverflowManager(parentContainer, data);
	return true;
} as JSWidgetHandler;
