...
class UserList extends L.Control {
	...
	onAddView(e: UserEvent) {
		...
		this.users.set(e.viewId, {you: you, username: username, extraInfo: e.extraInfo, color: color, readonly: e.readonly, following: false});
	...
	getColorLightness(hex: string) {
		// TODO: @minion3665 it might be nice to assert a few more of these assumptions?
		// We expect hex to be a hexidecimal color
		// It should have a hash to start
		// It may be either 3 or 6 digits long
	...
	renderFollowingChip() {
		...
		followingChip.innerHTML = this.options.followingChipText.replace('%user', following[1].username);
		followingChip.style.backgroundColor = following[1].color;

		const lightness = this.getColorLightness(following[1].color);

		if (lightness > 127) {
			followingChip.style.color = 'black';
		} else {
