...
class AboutDialog {
	...
	private static populateAboutDialog(elements: AboutDialogElements): void {
		...
		// WOPI Host ID
		elements.wopiHostId.textContent = windowAny.wopiHostId;

		// Copyright and vendor
		const span = document.createElement('span');
		span.setAttribute('dir', 'ltr');
		span.textContent = _(
			`Copyright © ${windowAny.copyrightYear}, ${windowAny.vendor}.`,
		);
		...
		if (windowAny.indirectSocket) {
			elements.routeToken.innerText = 'RouteToken: ' + windowAny.routeToken;
	...
	private copyVersionInfoToClipboard() {
		...
		addLine('WOPI host', windowAny.wopiHostId);
	...
	public static createAboutDialogContent(): AboutDialogElements {
		...
		const lokitExtra = AboutDialog.createElement('div', { id: 'lokit-extra' });
		infoDiv.appendChild(lokitExtra);
