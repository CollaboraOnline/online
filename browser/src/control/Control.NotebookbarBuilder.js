...
window.L.Control.NotebookbarBuilder = window.L.Control.JSDialogBuilder.extend({
        ...
        if (commandName === '.uno:CharFontName') {
            ...
            if (saveEle) {
                if (state === 'true' && this.map.saveState) {
                    ...
                    if (button)
                        button.classList.remove('savemodified');
                    var fileButton = document.getElementById('file-save');
                    if (fileButton)
    ...
    _createTabClick: function (builder, t, tabs, contentDivs, tabIds) {
        ...
        var tooltipCollapsed = isDesktop ? _('Click to expand') : _('Tap to expand');
        var tooltipExpanded = isDesktop ? _('Click to collapse') : _('Tap to collapse');
    ...
    // core side has to respect the children
    ...
    _getDownloadAsSubmenuOpts: function (docType) {
        ...
        if (docType === 'text') {
            ...
            submenuOpts = [
                ...
                : [])
                .concat(window.extraExportFormats.includes('impress_swf')
                ? [
                    {
                ...
                : [])
                .concat(window.extraExportFormats.includes('impress_svg')
                ? [
                    {
                ...
                : [])
                .concat(window.extraExportFormats.includes('impress_bmp')
                ? [
                    {
                ...
                : [])
                .concat(window.extraExportFormats.includes('impress_gif')
                ? [
                    {
                ...
                : [])
                .concat(window.extraExportFormats.includes('impress_png')
                ? [
                    {
                ...
                : [])
                .concat(window.extraExportFormats.includes('impress_tiff')
                ? [
                    {
    ...
    buildControl: function (parent, data) {
        if (!data.length)
            return;
        var inlineLabels = this.options.useInLineLabelsForUnoButtons;
        this.options.useInLineLabelsForUnoButtons = false;
        data = data[0];
        var type = data.type;
    ...
    build: function (parent, data, hasVerticalParent) {
        ...
        for (var childIndex in data) {
            ...
            var twoPanelsAsChildren = childData.children && childData.children.length == 2
...
//# sourceMappingURL=Control.NotebookbarBuilder.js.map