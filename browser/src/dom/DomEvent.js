...
class DomEventDerived extends DomEvent {
	...
	static on(obj, types, fn, context) {
		...
		if (typeof types === 'object') {
			for (let type in types) {
				DomEventDerived._on(obj, type, types[type], fn);
			}
		} else {
			types = app.util.splitWords(types);

			for (let i = 0, len = types.length; i < len; i++) {
				DomEventDerived._on(obj, types[i], fn, context);
			}
		}
	...
	static off(obj, types, fn, context) {
		...
		if (typeof types === 'object') {
			for (let type in types) {
				DomEventDerived._off(obj, type, types[type], fn);
			}
		} else {
			types = app.util.splitWords(types);

			for (let i = 0, len = types.length; i < len; i++) {
				DomEventDerived._off(obj, types[i], fn, context);
			}
		}
	...
	static _on(obj, type, fn, context) {
		...
		if (window.L.Browser.pointer && type.indexOf('touch') === 0) {
			DomEventDerived.addPointerListener(obj, type, handler, id);

		} else if ((type === 'dblclick') && DomEventDerived.addDoubleTapListener) {
			DomEventDerived.addDoubleTapListener(obj, window.touch.touchOnly(handler), id);

		} else if (type === 'trplclick' || type === 'qdrplclick') {
			DomEventDerived.addMultiClickListener(obj, handler, id, type);
	...
	static _off(obj, type, fn, context) {
		...
		if (window.L.Browser.pointer && type.indexOf('touch') === 0) {
			DomEventDerived.removePointerListener(obj, type, id);

		} else if ((type === 'dblclick') && DomEventDerived.removeDoubleTapListener) {
			DomEventDerived.removeDoubleTapListener(obj, id);

		} else if (type === 'trplclick' || type === 'qdrplclick') {
			DomEventDerived.removeMultiClickListener(obj, id, type);
	...
	static _skipped(e) {
		const skipped = DomEventDerived._skipEvents[e.type];
		// reset when checking, as it's only used in map container and propagates outside of the map
		DomEventDerived._skipEvents[e.type] = false;
		return skipped;
	}
