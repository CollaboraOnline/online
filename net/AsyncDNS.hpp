 ...
 */
/*
 * Asynchronous DNS resolution to avoid blocking the main event loop.
 *
 * AsyncDNS - Singleton that manages a background thread for DNS lookups.
 *            Queues lookup requests and invokes callbacks on the original
 *            SocketPoll thread when resolution completes.
 */
