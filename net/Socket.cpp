...
bool StreamSocket::checkRemoval(std::chrono::steady_clock::time_point now)
{
    ...
    const bool c2 = _pollTimeout > std::chrono::microseconds::zero() &&
                    durLast > _pollTimeout;
    const bool c3 = _minBytesPerSec > std::numeric_limits<double>::epsilon() &&
                    bytesPerSecIn > std::numeric_limits<double>::epsilon() &&
                    bytesPerSecIn < _minBytesPerSec;
    const bool c4 = SigUtil::getTerminationFlag();
    if (c1 || c2 || c3 || c4 )
    {
        LOG_WRN("CheckRemoval: Timeout: {c1 " << c1 << ", c2 " << c2 << ", c3 " << c3 << ", c4 " << c4 << "}, "
            << getStatsString(now) << ", "
            << *this);
        if (_socketHandler)
        {
