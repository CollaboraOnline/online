...
void Session::parseDocOptions(const StringVector& tokens, int& part, std::string& timestamp)
{
    ...
    for (std::size_t i = offset; i < tokens.size(); ++i)
    {
        ...
        {
            LOG_DBG("Processing signatureconfig parameter");

            if (_userPrivateInfo.empty())
            {
                LOG_WRN("signatureconfig: User private info not set, skipping signature configuration");
                ++offset;
                continue;
            }

            std::string decodedSignatureData = Uri::decode(value);
            LOG_DBG("signatureconfig: User info available, processing signature data");
            LOG_TRC("signatureconfig: Raw signature data = " << decodedSignatureData);
            ...
            {
                LOG_ERR("signatureconfig: Failed to parse signature data as JSON: " << decodedSignatureData);
                ++offset;
                continue;
            }
            ...
            {
                LOG_ERR("signatureconfig: Failed to parse user private info as JSON: " << _userPrivateInfo);
                ++offset;
                continue;
            }

            this->setSignToUserPrivateConfig("SignatureCert", signatureDataObject, userPrivateInfoObject);
            this->setSignToUserPrivateConfig("SignatureKey", signatureDataObject, userPrivateInfoObject);
            this->setSignToUserPrivateConfig("SignatureCa", signatureDataObject, userPrivateInfoObject);
