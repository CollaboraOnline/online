...
public:
    ...
    T elapsed(std::chrono::steady_clock::time_point now = std::chrono::steady_clock::now()) const
    {
...
public:
    ThreadCounter()
        : DirectoryCounter("/proc/self/task")
    {
    }
};
...
void replaceAllSubStr(std::string& input, const std::string& target,
...
{
    assert(subLen < std::numeric_limits<unsigned int>::max() && "Invalid sub-array length to find");
...
inline std::string_view eliminatePrefix(const std::string_view str, const std::string_view prefix)
{
...
inline std::pair<std::string, std::string>
splitLast(const char* s, const int length, const char delimiter = ' ', bool removeDelim = true)
{
...
/// Split a string in two at the delimiter, removing it.
inline std::pair<std::string, std::string>
splitLast(const std::string& s, const char delimiter = ' ', bool removeDelim = true)
{
...
template <typename Dst, typename Src, typename Enable = void> Dst convertChronoClock(const Src time)
{
...
{
    return getSystemClockAsString(convertChronoClock<std::chrono::system_clock::time_point>(time));
}
...
/**
     ...
     */
std::map<std::string, std::string> stringVectorToMap(const std::vector<std::string>& strvector,
                                                     char delimiter);
...
/// Convert a vector to a string. Useful for conversion in templates.
inline std::string toString(const std::vector<char>& x) { return std::string(x.data(), x.size()); }
...
template <std::size_t N> inline std::string toString(const char (&s)[N]) { return std::string(s); }
