...
bool ChildSession::insertFile(const StringVector& tokens)
{
    ...
    if constexpr (!Util::isMobileApp())
    {
        ...
        {
            sendTextFrameAndLogError("error: cmd=insertfile1 kind=syntax");
            return false;
        }
        ...
        if (type == "multimedia") {
            if (tokens.size() != 5 || !getTokenInteger(tokens[3], "width", multimedia_width) ||
                ...
                sendTextFrameAndLogError("error: cmd=insertfile2 kind=syntax");
                return false;
            }
        } else if (tokens.size() != 3) {
            sendTextFrameAndLogError("error: cmd=insertfile3 kind=syntax");
            return false;
        }
    }
    else
