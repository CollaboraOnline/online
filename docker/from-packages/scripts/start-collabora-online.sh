...
# Start coolwsd
exec /usr/bin/coolwsd --version --use-env-vars ${cert_params} --o:sys_template_path=/opt/cool/systemplate --o:child_root_path=/var/tmp/cool/child-roots --o:file_server_root_path=/usr/share/coolwsd --o:cache_files.path=/var/tmp/cool/cache --o:logging.color=false --o:stop_on_config_change=true ${extra_params} "$@"
