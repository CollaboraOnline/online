...
- (instancetype)initWithWebView:(WKWebView *)webView fileURL:(NSURL *)fileURL readOnly:(bool)readOnly {
    ...
    components.queryItems = @[ [NSURLQueryItem queryItemWithName:@"file_path" value:[fileURL absoluteString]],
                               ...
                               // TODO: [NSURLQueryItem queryItemWithName:@"lang" value:app_locale],
                               [NSURLQueryItem queryItemWithName:@"appdocid" value:[NSString stringWithFormat:@"%u", self.appDocId]],
                               [NSURLQueryItem queryItemWithName:@"userinterfacemode" value:@"notebookbar"],
                               // Related to issue #5841: the iOS app sets the
                               // base text direction via the "dir" parameter
                               // TODO: [NSURLQueryItem queryItemWithName:@"dir" value:app_text_direction],
                             ];
...
- (void)send2JS:(const char *)buffer length:(size_t)length {
    ...
    const char *posttext = "')});";
    const size_t pretextlen = strlen(pretext);
    const size_t posttextlen = strlen(posttext);
