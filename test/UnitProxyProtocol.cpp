...
public:
    UnitProxyProtocol()
        : UnitWSD("UnitProxyProtocol"),
          _poll(std::make_shared<SocketPoll>("client-poll")),
          _sentRequest(false)
    {
    ...
    void invokeWSDTest() override
    {
        ...
        helpers::getDocumentPathAndURL("empty.odt", documentPath, documentURL, "UnitProxyProtocol");

        auto httpSession = openSession();

        http::Request httpRequest(documentURL + "&compat=/ws/open/open/1");
        ...
        std::string loadCmd = documentURL;
        ...
        http::Request loadRequest(documentURL + "&compat=/ws/" + accessToken + "/write/2");
