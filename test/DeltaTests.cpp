...
void DeltaTests::testRleComplex()
{
    ...
    DeltaGenerator::DeltaData data(
        ...
        0, 0, 256, 256, TileLocation(1, 0, 0, 3840, 3840, 9, 9, 9), 256, 256);
...
void DeltaTests::testRleRandom()
{
    ...
    DeltaGenerator::DeltaData data(
        ...
        TileLocation(1, 0, 0, 3840, 3840, 9, 9, 9), 256, 256);

    // Compress
    std::vector<char> output;
    size_t size = gen.compressOrDelta(
        randomImg.data(), 0, 0, 256, 256, 256, 256,
        TileLocation(1, 0, 0, 3840, 3840, 42, 2, 3),
...
void DeltaTests::testDeltaSequence()
{
    ...
    LOK_ASSERT(gen.createDelta(
                       ...
                       TileLocation(1, 0, 0, 3840, 3840, 1, 2, 3), delta, textWid, false, LOK_TILEMODE_RGBA, rleData) == false);
    ...
    LOK_ASSERT(gen.createDelta(
                       ...
                       0, 0, width, height, width, height,
                       TileLocation(1, 0, 0, 3840, 3840, 1, 2, 3), delta, text2Wid, false, LOK_TILEMODE_RGBA, rleData) == true);
    ...
    LOK_ASSERT(gen.createDelta(
                       reinterpret_cast<unsigned char *>(&text[0]),
                       0, 0, width, height, width, height,
                       TileLocation(1, 0, 0, 3840, 3840, 1, 2, 3), two2one, textWid, false, LOK_TILEMODE_RGBA, rleData) == true);
